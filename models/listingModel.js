// DB helper functions for listings and related lookups
async function distinctPropertyTypes(db) {
    if (!db) return [];
    try {
        let types = await db.collection('listingsAndReviews').distinct('property_type');
...
async function distinctCountries(db) {
    if (!db) return [];
    try {
        let countries = await db.collection('listingsAndReviews').distinct('address.country');
